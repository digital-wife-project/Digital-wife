...
public class TestAnimationLoader : MonoBehaviour
{
    static string currentAnimationClipName;
    public GameObject baseModel;

    public void Creat_anim()
    {
        string anim_path= "\"C:\\Users\\wyy19\\Downloads\\无标题.fbx\"";
        Debug.Log("开始生成动画");
    ...
    private void OnAnimationModelLoad(AssetLoaderContext assetLoaderContext)
    {
        ...
        {
            AddAnimationAndPlay(assetLoaderContext);
        }
        else
        {
    ...
    private void AddAnimationAndPlay(AssetLoaderContext loadedAnimationContext)
    {
        // 获取加载的动画组件。
    ...
    /// 加载基础模型，包括所有模型数据。
    /// </summary>
    private void LoadBaseModel(GameObject baseModel)
    {
        // 获取基础模型的动画组件。
